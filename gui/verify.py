...
class Verify(QDialog, Base):
        ...
        if event.button() == Qt.LeftButton:
            ...
            if labelMinPos.x() <= cursorPos.x() <= labelMaxPos.x() and labelMinPos.y() <= cursorPos.y() <= labelMaxPos.y():
                ...
                if delIndex >= 0:
                    ...
                    newLabel.move(cursorPos.x() - labelPos.x() - pixmapSize.width() / 2,
                                  cursorPos.y() - labelPos.y() - pixmapSize.height() / 2)
                    newLabel.show()
    ...
    def getPoints(self):
        return ",".join([str(label.pos().x() + int(label.size().width() / 2)) + "," + str(
            label.pos().y() + int(label.size().height() / 2) - configGui.vcodeImageTipHeight) for label in
                         self.pointWidgets])
