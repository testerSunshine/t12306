...
class TiketInfo(Base):
    ...
    def __init__(self, parent=None):
        super(TiketInfo, self).__init__(parent)
        self.initBase()
        QThread.sleep(10)
    ...
    def getSearchLayout(self):
        ...
        for trainType in configCommon.trainTypes:
            ...
            self.conditionWidgets["trainTypes"][trainType["code"]] = self.triggerWidgets["trainTypes"][
                trainType["code"]] = cb
        ...
        for seatType in configCommon.seatTypes:
            if (seatType["des"]):
                ...
                self.conditionWidgets["seatTypes"][seatType["code"]] = self.triggerWidgets["seatTypes"][
                    seatType["code"]] = cb
    ...
    def getTrainDataLayout(self):
        ...
        labelNames = [u"车次", u"出发地" + os.linesep + u"目的地", u"出发时间" + os.linesep + u"到达时间", u"历时", u"商务座", u"特等座",
                      u"一等座", u"高等软座", u"软卧", u"硬卧", u"软座", u"硬座", u"无座", u"操作"]
